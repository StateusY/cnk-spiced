...
execute on passengers if entity @s[type=minecraft:item_display,tag=spiced.mortar_and_pestle_item] run data modify entity @n[type=item_display,tag=spiced.finished_mortar_and_pestle] item.components."minecraft:custom_data".spiced.last_recipe append from entity @s item
...
# execute if data entity @s data.spiced.last_interact_uuid run function spiced:mortar_and_pestle/post_mash/find with entity @s data.spiced