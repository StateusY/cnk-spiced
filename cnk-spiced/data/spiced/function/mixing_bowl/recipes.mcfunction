...
#         run return run data modify entity @s item.components."minecraft:custom_data".cnk.mix_callback set value "spiced:mixing_bowl/recipes/purple_dye"
...
execute if score $mixing_bowl_item_count cnk.dummy matches 2 \
        ...
        if data storage cnk:temp mixing_bowl.Items[{id:"minecraft:chicken"}] \
        if function cnk:mixing_bowl/mix/lock \
...
execute if score $mixing_bowl_item_count cnk.dummy matches 4 \
        if function spiced:mixing_bowl/generic/duplicate/milk if score $milk_count cnk.dummy matches 3 \
