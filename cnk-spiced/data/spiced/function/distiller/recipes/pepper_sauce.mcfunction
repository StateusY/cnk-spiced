...
data modify storage spiced:temp distiller.slot set from storage spiced:temp distiller.Items[{components:{"minecraft:custom_data":{spiced:{ingredient:{type:"paprika"}}}}}].Slot
function spiced:recipes/remove with storage spiced:temp distiller

data modify storage spiced:temp distiller.slot set from storage spiced:temp distiller.Items[{components:{"minecraft:custom_data":{spiced:{ingredient:{type:"diced_pepper"}}}}}].Slot
function spiced:recipes/remove with storage spiced:temp distiller
...
data modify storage spiced:temp distiller.color set value 8724739

# sets the callback function that will be called when a player uses a glass bottle on the basin with liquid in it
data modify storage spiced:temp distiller.callback set value "spiced:distiller/drinks/pepper_sauce/main"

# sets the liquid type, used in the liquid_check functions so must match
data modify storage spiced:temp distiller.liquid set value "pepper_sauce"
