...
data modify storage cnk:temp distiller.slot set from storage cnk:temp distiller.Items[{id:"minecraft:sugar"}].Slot
...
data modify storage cnk:temp distiller.color set value 10839296

# sets the callback function that will be called when a player uses a glass bottle on the basin with liquid in it
data modify storage cnk:temp distiller.callback set value "spiced:distiller/drinks/apple_cider_vinegar/main"

# sets the liquid type, used in the liquid_check functions so must match
data modify storage cnk:temp distiller.liquid set value "apple_cider_vinegar"
