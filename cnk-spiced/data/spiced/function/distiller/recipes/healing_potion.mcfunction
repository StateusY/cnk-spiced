...
data modify storage spiced:temp distiller.slot set from storage spiced:temp distiller.Items[{id:"minecraft:nether_wart"}].Slot
function spiced:recipes/remove with storage spiced:temp distiller

data modify storage spiced:temp distiller.slot set from storage spiced:temp distiller.Items[{id:"minecraft:glistering_melon_slice"}].Slot
function spiced:recipes/remove with storage spiced:temp distiller
...
data modify storage spiced:temp distiller.color set value 16262179

# sets the callback function that will be called when a player uses a glass bottle on the basin with liquid in it
data modify storage spiced:temp distiller.callback set value "spiced:distiller/drinks/healing_potion/main"

# sets the liquid type, used in the liquid_check functions so must match
data modify storage spiced:temp distiller.liquid set value "healing_potion"
