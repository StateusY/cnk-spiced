...
data modify storage cnk:temp distiller.slot set from storage cnk:temp distiller.Items[{components:{"minecraft:custom_data":{spiced:{ingredient:{type:"soybeans"}}}}}].Slot
function cnk:recipes/remove with storage cnk:temp distiller
...
data modify storage cnk:temp distiller.color set value 2097155

# sets the callback function that will be called when a player uses a glass bottle on the basin with liquid in it
data modify storage cnk:temp distiller.callback set value "spiced:distiller/drinks/soy_sauce/main"

# sets the liquid type, used in the liquid_check functions so must match
data modify storage cnk:temp distiller.liquid set value "soy_sauce"
