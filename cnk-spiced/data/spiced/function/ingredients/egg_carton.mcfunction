execute store result score $count spiced.dummy if items entity @s container.* poisonous_potato[minecraft:custom_data~{"spiced": {"ingredient":{"type":"egg"}}}]
...
execute as @a[tag=summit.in_booth.spiced,distance=..15] run function spiced:ingredients/sfx/egg
