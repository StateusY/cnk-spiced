$dialog show @s { \
  "type": "minecraft:multi_action", \
  "title": {"translate":"item.cnk.cookbook"}, \
  "body": [ \
    { \
      "type": "minecraft:plain_message", \
      "width": 211, \
      "contents": [{ \
            "translate":"book.cnk.5_recipe","font":"cnk.book:base","shadow_color":0,"color":"white","with":[ \
                {"translate":item.cnk.cheeseburger,"color":"#7b613a","font":"minecraft:default"}, \
                {"translate":book.cnk.ingredient_template,with:[{"translate":book.item.minecraft.bread,"font":"cnk.book:icons"}],"hover_event":{"action":"show_text","value":[{"translate":item.minecraft.bread},{'translate':book.cnk.hyperlink,'font':'cnk.book:base'}]},'click_event':{'action':'run_command','command':'trigger cnk.cookbook_references set 9'}}, \
                {"translate":book.cnk.ingredient_template,with:[{"translate":book.item.cnk.beef_cutlets,"font":"cnk.book:icons"}],"hover_event":{"action":"show_text","value":[{"translate":item.cnk.beef_cutlets},{'translate':book.cnk.hyperlink,'font':'cnk.book:base'}]},'click_event':{'action':'run_command','command':'trigger cnk.cookbook_references set 3'}}, \
                {"translate":book.cnk.ingredient_template,with:[{"translate":book.item.cnk.lettuce,"font":"cnk.book:icons"}],"hover_event":{"action":"show_text","value":{"translate":item.cnk.lettuce}}}, \
                {"translate":book.cnk.ingredient_template,with:[{"translate":book.item.cnk.tomato,"font":"cnk.book:icons"}],"hover_event":{"action":"show_text","value":{"translate":item.cnk.tomato}}}, \
                {"translate":book.cnk.ingredient_template,with:[{"translate":book.item.cnk.cheese,"font":"cnk.book:icons"}],"hover_event":{"action":"show_text","value":[{"translate":item.cnk.cheese},{'translate':book.cnk.hyperlink,'font':'cnk.book:base'}]},'click_event':{'action':'run_command','command':'trigger cnk.cookbook_references set 5'}}, \
                {"translate":"book.cnk.cooking_pot"}, \
                {"translate":"book.item.cnk.cheeseburger","font":"cnk.book:icons"}, \
                {"translate":book.tab.cnk.front.idle,"hover_event":{"action":"show_text","value":{"translate":book.cnk.section.front}},"click_event":{"action":"run_command","command":"trigger cnk.cookbook_buttons set 9921"}}, \
                {"translate":book.tab.cnk.staple.idle,"hover_event":{"action":"show_text","value":{"translate":book.cnk.section.staple}},"click_event":{"action":"run_command","command":"trigger cnk.cookbook_buttons set 9922"}}, \
                {"translate":book.tab.cnk.snack.idle,"hover_event":{"action":"show_text","value":{"translate":book.cnk.section.snack}},"click_event":{"action":"run_command","command":"trigger cnk.cookbook_buttons set 9923"}}, \
                {"translate":book.tab.cnk.light.idle,"hover_event":{"action":"show_text","value":{"translate":book.cnk.section.light}},"click_event":{"action":"run_command","command":"trigger cnk.cookbook_buttons set 9924"}}, \
                {"translate":book.tab.cnk.hearty.active,"hover_event":{"action":"show_text","value":{"translate":book.cnk.section.hearty}},"click_event":{"action":"run_command","command":"trigger cnk.cookbook_buttons set 9925"}}, \
                {"translate":book.tab.cnk.feast.idle,"hover_event":{"action":"show_text","value":{"translate":book.cnk.section.feast}},"click_event":{"action":"run_command","command":"trigger cnk.cookbook_buttons set 9926"}}, \
                {"translate":book.tab.cnk.dessert.idle,"hover_event":{"action":"show_text","value":{"translate":book.cnk.section.dessert}},"click_event":{"action":"run_command","command":"trigger cnk.cookbook_buttons set 9927"}}, \
                {"translate":book.cnk.stamp.plateable.icon,"hover_event":{"action":"show_text","value":{"translate":book.cnk.stamp.plateable.text}}}, \
                {"translate":$(return)}, \
                {"translate":book.cnk.bottom_line,"with":[{"translate":book.cnk.page_number.$(page_number_width),"font":"cnk.book:small_text","with":[{"text":"$(current_page)","color":"#927359"}]}]}, \
                {"translate":book.cnk.cookbook_navigation,"with":[{"translate":book.cnk.previous_page,"hover_event":{"action":"show_text","value":{"translate":book.cnk.previous_page.hover}},"click_event":{"action":"run_command","command":"trigger cnk.cookbook_buttons set 9901"}},{"translate":book.cnk.empty_incomplete_recipe},{"translate":book.cnk.source,"font":"cnk.book:base","hover_event":{"action":"show_text","value":{"translate":cnk.source}}},{"translate":book.cnk.empty_incomplete_recipe},{"translate":book.cnk.next_page,"hover_event":{"action":"show_text","value":{"translate":book.cnk.next_page.hover}},"click_event":{"action":"run_command","command":"trigger cnk.cookbook_buttons set 9902"}}]} \
            ] \
        }] \
    }, \
    { \
      "type": "minecraft:plain_message", \
      "width": 1, \
      "contents": [{"text":""}] \
    } \
  ], \
  "inputs": [], \
  "can_close_with_escape": true, \
  "pause": false, \
  "after_action": "none", \
  "actions": [ \
    { \
      "label": {translate:"gui.done"}, \
      "width": 211, \
      "action": { \
        "type": "run_command", \
        "command": "trigger cnk.cookbook_buttons set 9991" \
      } \
    } \
  ] \
}